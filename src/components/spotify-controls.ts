...
export class SpotifyControls {
    ...
    constructor() {
        ...
        const extension = extensions.getExtension('krisnarhesa.resound') ||
            extensions.all.find(e => e.packageJSON && e.packageJSON.name === 'resound');
        if (!extension) {
            this._buttons = [];
            return;
        }
        ...
        this._buttons = buttonsInfo.map(item => {
            ...
            const buttonCommand = 'resound.' + item.id;
