...
export class LyricsController {
    ...
    readonly registration = workspace.registerTextDocumentContentProvider('resound', LyricsController.lyricsContentProvider);

    private readonly previewUri = Uri.parse('resound://authority/resound');
    ...
    private async _findLyrics() {
        ...
        try {
            ...
            try {
                ...
                // Genius uses multiple containers for lyrics and <br> for newlines
                $('[data-lyrics-container="true"]').find('br').replaceWith('\n');
                let lyrics = '';
                $('[data-lyrics-container="true"]').each((_, el) => {
                    // text() will now preserve the \n we inserted
                    lyrics += $(el).text().trim() + '\n\n';
                });
                
                lyrics = lyrics.trim();
                
                // Fallback to legacy class if empty
                if (!lyrics) {
