...
### Fixed
- Fixed missing Track Info metadata when playing Podcast Episodes.
- Set Spotify Web API as the default client for all platforms (Windows, Linux, macOS) so remote device playback (phones, browsers) is detected out of the box.

## [1.1.4] - 2026-09-03

### Fixed

- Fixed a critical "Null Pointer Exception" that caused the "Now Playing" status to crash when playing Music, Local Files, or during the first second of a track transition. Handled safely using optional chaining.
